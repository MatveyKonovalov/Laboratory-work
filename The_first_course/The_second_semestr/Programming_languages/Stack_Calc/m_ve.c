...
void prbp()
{
    ...
    for (int i = 0; i < sp; i++)
    {
        // Проверяем, является ли число целым (с допустимой погрешностью)
...
void infix_to_rpn(const char *infix, char *rpn)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        // Если число, десятичная точка или знак минус у числа
        if (isdigit(c) || c == '.')
        {
...
// Вычисление RPN выражения (поддержка вещественных чисел)
double evaluate_rpn(const char *rpn)
{
    ...
    for (i = 0; rpn[i] != '\0'; i++)
    {
        if (rpn[i] == ' ')
        {
            ...
            {
                token[token_index] = '\0';
                // Проверяем, является ли токен числом (включая отрицательные и вещественные)
...
// Интерактивный калькулятор RPN с поддержкой вещественных чисел
void rpn_calculator()
{
    char c;
    
    printf("RPN Calculator Mode (supports real numbers like 3.14)\n");
    printf("Enter numbers and operators (e.g., 5 3.5 + 2.5 / =)\n");
...
int main()
{
    ...
    printf("=== RPN Calculator & Compiler (Real Numbers Support) ===\n");
    ...
    if (choice == 1)
    {
        printf("Enter infix expression (supports real numbers, e.g., 3.5+4.2*2/(1.5-5)^2^3): ");
